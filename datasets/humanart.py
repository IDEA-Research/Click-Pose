...
class CocoDetection(torch.utils.data.Dataset):
    def __init__(self, root_path, image_set, transforms, return_masks):
        ...
        if image_set == "train":
            ...
            self.coco = COCO(root_path / "HumanArt/annotations/training_humanart.json")
            ...
            self.img_folder = root_path
            self.coco = COCO(root_path / "HumanArt/annotations/validation_humanart.json")
            imgIds = sorted(self.coco.getImgIds())
            self.all_imgIds = []
            for image_id in imgIds:
                if os.environ.get("CLICKPOSE_NoC_Test") == "TRUE":
                    if self.coco.getAnnIds(imgIds=image_id) == []:
                        continue
                    ann_ids = self.coco.getAnnIds(imgIds=image_id)
                    target = self.coco.loadAnns(ann_ids)
