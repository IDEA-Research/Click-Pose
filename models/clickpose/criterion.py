...
class SetCriterion(nn.Module):
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, num_box_decoder_layers=2,
                 num_body_points=17):
        super().__init__()
    ...
    def loss_dn_keypoints(self, outputs, targets, indices, num_boxes):
        """
        Input:
            - src_keypoints: bs, num_dn, 51
            - tgt_keypoints: bs, num_dn, 51
        ...
        if 'num_tgt' not in outputs:
            device = outputs["pred_logits"].device
            losses = {
                'dn_loss_keypoints': torch.as_tensor(0., device=device),
            }
            return losses
        num_tgt = outputs['num_tgt']
        src_keypoints = outputs['dn_pose_pred']
        tgt_keypoints = outputs['dn_pose_input']

        return self.tgt_loss_keypoints(src_keypoints, tgt_keypoints, num_tgt)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "dn_keypoints": self.loss_dn_keypoints,
    ...
    def forward(self, outputs, targets, return_indices=False):
            ...
            for idx, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss in ['keypoints'] and idx < self.num_box_decoder_layers:
                        continue
                    if loss in ['dn_keypoints'] and idx < self.num_box_decoder_layers:
                        continue
                    kwargs = {}
                    if loss == 'labels':
        ...
        if 'interm_outputs' in outputs:
            ...
            if return_indices:
                indices_list.append(indices)
            for loss in self.losses:
                if loss in ['dn_bbox', 'dn_label', 'keypoints', "dn_keypoints"]:
                    continue
                kwargs = {}
                if loss == 'labels':
                    kwargs = {'log': False}
    ...
    def tgt_loss_keypoints(self, src_keypoints, tgt_keypoints, num_tgt, ):
        """
        Input:
            - src_keypoints: bs, num_dn, 51
            - tgt_keypoints: bs, num_dn, 51
        """
        loss_keypoints = F.l1_loss(src_keypoints[..., :34], tgt_keypoints[..., :34], reduction='none')
        loss_keypoints = loss_keypoints * tgt_keypoints[..., 34:].repeat_interleave(2, dim=-1)
        losses = {}
        losses['dn_loss_keypoints'] = loss_keypoints.sum() / num_tgt
    ...
    def tgt_loss_labels(self, src_logits: Tensor, tgt_labels: Tensor, num_tgt: int, log: bool = True):
        ...
        target_classes_onehot = target_classes_onehot[:, :, :-1]
        loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_tgt, alpha=self.focal_alpha, gamma=2) * \
                  src_logits.shape[1]
